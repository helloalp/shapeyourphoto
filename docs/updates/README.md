...
从 1.2.7 起，需要提交并用于同步检查的结构化更新历史维护在 `config/changelog/versions.json`；`data/` 只保存本地运行数据并整体排除在版本控制之外。
...
4. 不得用交接记录或聊天摘要替代版本文档。
5. 不得在版本文档中写入开发者设备型号、非公开测试环境、内部依赖栈、AI 协作过程、提示词、维护者操作过程或适用对象标签；版本记录只写用户可理解的功能、体验和安全结果。
