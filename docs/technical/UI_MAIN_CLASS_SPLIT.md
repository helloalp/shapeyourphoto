...
- 内部字段、枚举、issue code、repair method id 和存储结构继续使用英文；UI 通过 `src/ui/display_names.py` 做中文化展示，不反向写入逻辑判断。
...
`src/ui_*` mixin 仍保留为兼容层，避免一次性大搬迁造成循环 import 或窗口打不开。后续迁移应按职责逐步移动到 `src/ui/mixins/` 或更具体的包内模块，并保持薄封装一到两个版本。

布局构建 `_build_ui()` 仍留在 `src/ui_app.py`，但 1.1.7 已把标题、主题、HiDPI、Splash、显示名和 EXIF 编辑移入 `src/ui/`。后续可继续拆出主界面布局 builder。
