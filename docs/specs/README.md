...
本目录中的产品与 UI 规范不得限制产品未来发展方向、适用人群、使用场景、AI 能力接入或专业化功能扩展；任何修改、增删、更新、升级都只能记录当前已实现能力、当前用户体验要求和安全约束。
...
- 设置窗口保存后不关闭，底部用简短状态说明保存结果，用户可继续调整。
- 右侧预览图首次显示不能停留在小图状态，应在布局稳定后按可用区域重绘；超高清原图预览不得在主线程反复完整解码。
