...
- 启动入口继续向原生 `ShapeYourPhoto.exe` 收敛，源码兼容入口 `start.bat` 不再作为普通发布包的推荐启动方式。
- 运行依赖从根目录移动到 `requirements/runtime.txt`，启动 helper 会优先读取新路径并兼容旧源码布局。
- 批量处理目标现在严格以当前列表和当前选择为准，移出列表的图片不会被分析、修复或转换重新拉回。
- EXIF 编辑页区分文件名、可编辑项、只读项和软件保留项；标题、作者、版权、关键词和备注可保存，文件名仅显示。
- 不适合保留的图片复核窗口改为多图大预览，显示文件名、严重程度、主要原因和选择状态，不再显示置信度。
- 相似图片复核窗口会根据可用宽度并列显示 2 到 4 张图片，减少宽屏下不必要的翻页和滚动。
- Console 时间部分改为更醒目的样式，批量日志继续合并刷新，复制内容仍保持纯文本。
- 暗背景人像的欠曝判断更稳健，脸部曝光正常时不会因大面积暗背景直接进入严重不适合保留候选。
- 批量分析统计改为每轮结束时统一保存，不再每完成一张图片就加密并写入统计文件，降低大批量分析期间 UI 卡顿和磁盘写入压力。
- 文件夹扫描进度在后台扫描线程先做节流，再投递到 UI 队列，避免大目录逐文件回调把主线程队列撑满。
- 分析耗时统计移除重复包裹的 `basic_stats` 阶段，Console 和 benchmark 的慢阶段排行会指向曝光、色彩、清晰度、人像等真实耗时来源。
- 本地 benchmark 输出新增平均排队等待/张，并保留累计排队等待，单线程或低并发基线不再把累计值误读成真实等待时间。
- 批量格式转换的后台 worker 改为通过主线程 UI 队列更新进度和完成状态，避免窗口关闭或转换收尾时从后台线程直接调用 Tk。
- 递归文件夹扫描移除 `os.walk` 文件列表里的重复 `is_file()` 检查，减少大目录导入时的逐文件 stat 开销。
- 不适合保留复核窗口改为先显示列表，再后台解码缩略图并分批贴回 UI，候选图片较多时打开窗口不再等待所有缩略图生成完成。
- 相似图片组复核窗口改为每次渲染使用一个后台 worker 解码缩略图，筛选结果较多时不再同步等待所有预览缩略图生成。
- 相似图片抉择窗口翻页时先显示操作卡片和分析摘要，再后台解码较大的对比预览图，减少高分辨率图片翻页卡顿。
- 主 UI 回调队列在窗口关闭和后台任务收尾同时发生时会安全停止，不再在 Tcl 对象销毁后继续安排回调。
- 随包 Rust GPU core 版本号同步到 1.2.7，native 自检和诊断输出与应用版本保持一致。
- 新增第一版原生 Rust launcher；`start.bat` 在已构建时会优先调用它，再由它定位 Python 并委托现有启动 helper，保留当前 Python/Tk 全部能力。
- 新增 Rust 原生化路线文档，并把本轮发现的 Tk 线程边界、进度节流、缩略图异步、统计写盘和 benchmark 计时口径规则同步到技术规范。
- 相似图片特征提取会先保留原始方向后的尺寸，再把工作图限制到固定上限，减少大 PNG 或无法使用 JPEG draft 的图片在相似检测阶段造成的 CPU 和内存峰值。
- 修复候选评分不再先调用未参与评分的 native GPU 全局亮度统计再重复计算 CPU 区域指标；候选安全检查会复用本轮已算出的原图和候选图 metrics。
- 修复入口恢复“缺少分析结果时先补分析再修复”的流程；目标校验不再提前丢弃仍在当前列表内、文件存在但尚未分析的图片。
- 更新器窗口的后台 worker 不再直接调用 Tk `after()`；成功重启、取消提示和失败回滚弹窗都改为通过主线程 UI 队列执行。
...
- 切换语言并保存后，主窗口菜单、顶栏按钮、筛选下拉、列表/清理表头、右侧标签页和设置窗口框架会立即刷新，不再要求重启后才看到主要文案变化。
- 修复当前 Tk 版本下顶层菜单项不支持直接改 `label` 时导致主窗口初始化或语言刷新崩溃的问题；语言刷新改为重建菜单栏，保留原有菜单命令。
...
- 右侧大图预览改为后台解码并合并快速切换请求，快速连续点选超高清图片时不再让 Tk 主线程等待完整预览解码。
...
- 明确源码包需要 Python 运行环境；`start.bat` 会保留提示窗口并给出下一步说明。
...
- 设置页新增语言偏好保存逻辑；更新页只显示 `Shape Your Photo vX.Y.Z` 和“版本 ID N”，不再展示构建 ID、更新 URL、公告 URL 或自动管理说明。
...
- 独立 updater 在包大小或 sha256 校验失败时输出 expected/actual/url 细节，并先切换为可关闭状态再显示失败提示；`package_size` 不一致但 sha256 正确时会继续更新。
...
- 设置页更新页面新增“检查更新”按钮，并只显示当前版本名称和版本 ID，不再展示构建 ID。
- 设置页文案改为面向用户的简短说明，减少实现细节。
- 主窗口标题改为 `Shape Your Photo | v1.1.9 | by Helloalp`。
- README 改为快速启动入口，并引导到 `docs/` 查看维护规则；macOS 当前说明改为后续补充。
...
- 明确版本记录语言规范：`app_metadata.CHANGELOG`、`docs/CHANGELOG.md` 和 `docs/updates/<version>.md` 默认使用中文；英文仅保留在文件名、函数名、协议字段、环境变量和内部 code/enum 等技术标识中。
...
- 同步更新 `app_metadata.py` 内置 CHANGELOG、`docs/CHANGELOG.md`、`README.md` 当前版本和 `docs/updates/1.1.7.md`，把 main 修改与本地修改都写入 1.1.7 版本说明。
...
- 应用设置新增分析并发模式（自动/低/中/高/自定义 worker 数）和 GPU 加速模式（关闭/自动/开启）；加速组件未准备或设备不可用时自动回退 CPU。
